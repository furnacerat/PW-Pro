...
struct AddNoteView: View {
    ...
    private func generateAISuggestion() {
        ...
        Task {
            do {
                let history = client.interactions.map { "\($0.date): \($0.notes)" }.joined(separator: "\n")
                let suggestion = try await GeminiManager.shared.generateSuggestedResponse(
                    for: client.name,
                    history: history,
                    type: type.rawValue
                )
                ...
                print("Gemini suggestion failed: \(error)")
                await MainActor.run {
                    isSuggesting = false
                }
            }
