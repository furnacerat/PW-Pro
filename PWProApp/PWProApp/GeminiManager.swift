...
class GeminiManager: ObservableObject {
    ...
    private func setupModel() {
        guard let apiKey = ConfigurationService.shared.geminiKey else {
            print("ERROR: GEMINI_API_KEY not found in Configuration Service")
    ...
    func generateSuggestedResponse(for clientName: String, history: String, type: String) async throws -> String {
        guard let model = model else { throw NSError(domain: "GeminiManager", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model not initialized"]) }
        
        let prompt = "Generate a short, professional \(type) message for a client named \(clientName). Their history: \(history). Keep it concise and friendly."
        let response = try await model.generateContent(prompt)
        return response.text ?? ""
    }
    
    func optimizeRoute(jobs: [(id: UUID, address: String)]) async throws -> [UUID] {
        guard let model = model else { throw NSError(domain: "GeminiManager", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model not initialized"]) }
        
        let addresses = jobs.map { "\($0.id): \($0.address)" }.joined(separator: "\n")
        let prompt = """
        Optimize the following job route by re-ordering the UUIDs to minimize travel time between addresses.
        Return ONLY a comma-separated list of UUID strings in the optimal order.
