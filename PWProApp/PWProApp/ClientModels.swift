...
struct Client: Identifiable, Codable {
    ...
    var rating: Double {
        // Simple rating based on job count and status
        let baseRating = jobHistory.count > 0 ? 4.0 : 3.0
        return status == .vip ? 5.0 : (status == .regular ? baseRating : 3.0)
    }
    
    var totalSpent: Double {
        totalLifetimeValue // Alias for totalLifetimeValue
    }
