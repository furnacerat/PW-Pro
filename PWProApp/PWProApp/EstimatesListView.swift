...
struct EstimatesListView: View {
    ...
    var body: some View {
        ...
        .task {
            if estimateManager.estimates.isEmpty {
                await estimateManager.fetchEstimates()
            }
        }
        .alert("Delete Estimate?", isPresented: $showingDeleteConfirmation) {
            ...
            Button("Delete", role: .destructive) {
                if let estimate = estimateToDelete {
                    // Call delete on manager (need to verify delete method exists/is exposed)
                    estimateManager.deleteEstimate(id: estimate.id)
                    estimateToDelete = nil
                }
            }
        } message: {
